...
impl<T: AsRef<[u8]>> Packet<T> {
    ...
    pub fn mcast_addr(&self) -> Ipv6Address {
        let data = self.buffer.as_ref();
        crate::wire::ipv6_from_octets(data[field::QUERY_MCAST_ADDR].try_into().unwrap())
    }
...
impl<T: AsRef<[u8]>> AddressRecord<T> {
    ...
    pub fn mcast_addr(&self) -> Ipv6Address {
        let data = self.buffer.as_ref();
        crate::wire::ipv6_from_octets(data[field::RECORD_MCAST_ADDR].try_into().unwrap())
    }
}
...
mod test {
    ...
    fn test_query_deconstruct() {
        ...
        assert_eq!(
            crate::wire::ipv6_from_octets(packet.payload().try_into().unwrap()),
    ...
    fn test_record_deconstruct() {
        ...
        assert_eq!(
            crate::wire::ipv6_from_octets(addr_rcrd.payload().try_into().unwrap()),
            IPV6_LINK_LOCAL_ALL_ROUTERS
        );
    }
