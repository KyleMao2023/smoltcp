...
impl<T: AsRef<[u8]>> Packet<T> {
    ...
    pub fn group_addr(&self) -> Ipv4Address {
        let data = self.buffer.as_ref();
        crate::wire::ipv4_from_octets(data[field::GROUP_ADDRESS].try_into().unwrap())
    }
...
mod test {
    ...
    fn test_leave_group_deconstruct() {
        ...
        assert_eq!(
            packet.group_addr(),
            crate::wire::ipv4_from_octets([224, 0, 6, 150])
        );
    ...
    fn test_report_deconstruct() {
        ...
        assert_eq!(
            packet.group_addr(),
            crate::wire::ipv4_from_octets([225, 0, 0, 37])
        );
        assert!(packet.verify_checksum());
    }
    ...
    fn test_leave_construct() {
        ...
        packet.set_msg_type(Message::LeaveGroup);
        packet.set_max_resp_code(0);
        packet.set_group_address(crate::wire::ipv4_from_octets([224, 0, 6, 150]));
        packet.fill_checksum();
    ...
    fn test_report_construct() {
        ...
        packet.set_max_resp_code(0);
        packet.set_group_address(crate::wire::ipv4_from_octets([225, 0, 0, 37]));
        packet.fill_checksum();
